...
const FooterContentLinksNavigation = (): React.ReactElement => {
   return (
         ...
         <ul className="footer-content__links-navigation-list flex flex-col gap-2.5">
            {FOOTER_CONTENT_LINKS_NAVIGATION_LINKS.map((link) => (
               <li key={link.HREF} className="footer-content__links-navigation-list-item">
                  <a href={link.HREF} aria-label={link.ARIA_LABEL} className="footer-content__links-navigation-link">
                     <span className="footer-content__links-navigation-link-text relative group text-sm font-bold cursor-pointer text-heading">
                        {link.LABEL}
                        <span className="footer-content__links-navigation-link-underline absolute left-0 bottom-0 h-px bg-current w-0 group-hover:w-full transition-all duration-300"></span>
                     </span>
                  </a>
               </li>
            ))}
