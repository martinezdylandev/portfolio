   ...
   return (
      <div className="relative">
         <button ref={buttonRef} className="footer__navigation-compact__menu-button w-10 h-10 flex items-center justify-center" onClick={handleClick} aria-label={BUTTON_ARIA_LABEL} aria-expanded={isNavigationMenuOpen} aria-controls="footer-navigation-menu">
            <img src={theme === "light" ? "/header/hamburguer_icon_light_mode.svg" : "/header/hamburguer_icon_dark_mode.svg"} className={`footer__navigation-compact__menu-icon w-6 h-6 transition-transform duration-100 ease-in-out ${isNavigationMenuOpen ? "rotate-90" : "rotate-0"}`} alt={ICON_ALT_LABEL} />
         </button>
         <AnimatePresence>
            {isNavigationMenuOpen && (
               <motion.nav ref={navRef} id="footer-navigation-menu" tabIndex={-1} variants={navMenuVariants} initial="closed" animate="open" exit="closed" transition={{ duration: 0.2, ease: "easeInOut" }} className="footer__navigation-compact absolute bottom-full mb-2" aria-label={NAV_ARIA_LABEL_TEXT}>
