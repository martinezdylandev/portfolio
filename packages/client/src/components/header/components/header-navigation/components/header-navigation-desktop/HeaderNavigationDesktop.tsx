...
const HeaderNavigationDesktop = (): React.ReactElement => {
   return (
      <nav className="header-navigation-desktop hidden lg:flex items-center" aria-label={NAV_ARIA_LABEL_TEXT}>
         <ul className="header-navigation-desktop__list flex justify-center gap-5" aria-label={LIST_ARIA_LABEL_TEXT}>
            {HEADER_DESKTOP_NAVIGATION_LINKS.map((link, index) => (
               <li key={index} className="header-navigation-desktop__item">
                  <a href={link.HREF} className="header-navigation-desktop__link">
