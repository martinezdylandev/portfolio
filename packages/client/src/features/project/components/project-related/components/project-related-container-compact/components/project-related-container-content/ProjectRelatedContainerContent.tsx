...
const ProjectRelatedContainerContent = (): React.ReactElement => {
   ...
   return (
      <div className={`project__related-container-content flex flex-col px-6`} aria-label={PROJECT_RELATED_CONTAINER_CONTENT_ARIA_LABEL}>
         <h2 className={`project__related-container-content-title font-hanken-grotesk font-bold text-center text-heading mb-6 text-display-sm`}>{PROJECT_RELATED_CONTAINER_CONTENT_TITLE}</h2>
         ...
         <Link to={`/projects/`} className="project__related-container-content-link text-center mt-12.5" aria-label={PROJECT_RELATED_CONTAINER_CONTENT_LINK_ARIA_LABEL}>
            <motion.span initial={initial} whileInView={whileInView} transition={transition} viewport={viewport} className="project__related-container-content-text font-hanken-grotesk text-heading-sm font-bold text-heading">
