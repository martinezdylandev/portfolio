...
const ProjectTimelineContainerDesktop = ({ project }: ProjectTimelineContainerProps): React.ReactElement => {
   const { initial, whileInView, transition, viewport } = useProjectTimelineContainerAnimations();

   return (
      <motion.div initial={initial} whileInView={whileInView} transition={transition} viewport={viewport} className="project__timeline-container project__timeline-container--desktop flex flex-col p-25 min-h-screen" aria-label={PROJECT_TIMELINE_CONTAINER_ARIA_LABEL}>
         <h2 className="project__timeline-container-title font-hanken-grotesk text-display-lg leading-none font-bold mb-25 text-heading">
...
export default ProjectTimelineContainerDesktop;
