...
const ProjectMainCoverContainerDesktop = ({ project }: ProjectMainCoverContainerProps) => {
   const { initial, whileInView, transition, viewport } = useProjectMainCoverContainerAnimations();

   return (
      <motion.div initial={initial} whileInView={whileInView} transition={transition} viewport={viewport} className="project__main-cover-container grid gap-24 p-25 min-h-screen content-center" aria-label={PROJECT_MAIN_COVER_CONTAINER_ARIA_LABEL}>
