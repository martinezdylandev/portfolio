      ...
      <motion.li initial={{ x: isOddProject ? -700 : 700, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.1, ease: "easeInOut" }} viewport={{ once: true, amount: 0.1 }} className="projects-list-detailed__list-item projects-list-detailed__list-item--desktop grid min-h-screen cursor-pointer rounded-xs border-0 group bg-card p-12 transition duration-500 ease-in-out hover:bg-accent" aria-label={`Project list item: ${project.project_name}`}>
         <Link className="projects-list-detailed__link contents" to={`/projects/${project.project_name}`} aria-label={`View ${project.project_name}`}>
            <div className="flex items-center justify-between py-12.5">
               <div className={`projects-list-detailed__first-wrapper max-w-3/4 ${isOddProject ? "order-1 text-left" : "order-2 text-right"}`} aria-label={`Project ${project.project_name} details`}>
                  <h2 className="text-8xl font-bold transition-colors duration-300 text-heading group-hover:text-white">{firstPartName}</h2>
                  <h3 className="text-6xl font-bold transition-colors duration-300 text-heading group-hover:text-white">{secondPartName}</h3>
               ...
               <div className={`projects-list-detailed__second-wrapper size-75 flex items-center justify-center rounded-[50%] transition-colors duration-300 bg-card group-hover:bg-page ${isOddProject ? "order-2" : "order-1"}`} aria-label={`Project ${project.project_name} logo`}>
